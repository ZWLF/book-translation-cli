...
from booksmith.chaptering.manual_toc import load_manual_toc_titles
...
from booksmith.output import assembler as output_assembler
from booksmith.output import epub_renderer
from booksmith.output import polished_pdf as polished_pdf_module
...
from booksmith.publishing import editorial_revision, layout_review, source_audit
from booksmith.publishing import structure as structure_module
from booksmith.publishing import validation as output_validation
from booksmith.publishing.deep_review import (
    DEEP_REVIEW_STAGE_VERSION,
    run_deep_review,
)
from booksmith.publishing.final_review import (
    FINAL_REVIEW_STAGE_VERSION,
    apply_final_review,
)
from booksmith.publishing.lexicon import merge_lexicon_overrides, normalize_lexicon_records
from booksmith.publishing.proofread import PROOFREAD_STAGE_VERSION, proofread_chapter
from booksmith.publishing.release_gate import evaluate_release_gate
...
from booksmith.utils import file_fingerprint, slugify
...
async def _ensure_deep_review_stage(
    *,
    ...
    working_final_artifacts = final_artifacts or _load_publishing_artifacts(
        workspace.publishing_final_chapters_path,
        workspace,
    )
    ...
    result = run_deep_review(
        ...
        final_artifacts=working_final_artifacts,
    ...
    await _rebuild_stable_publishing_outputs(
        workspace=workspace,
        manifest=manifest,
        chapters=working_final_artifacts,
        deep_review_book=result.structured_book,
...
def _final_chapters_fingerprint(workspace: Workspace) -> str:
    if not workspace.publishing_final_chapters_path.exists():
        return "missing"
    return _fingerprint_payload(
        workspace.read_publishing_jsonl(workspace.publishing_final_chapters_path)
    )
