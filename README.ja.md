...
デスクトップ GUI は、CLI と同じ翻訳パイプラインの上にある独立したローカルの入口です。CLI の代わりではなく、補完するためのものです。

GUI は、対話的に使いたいときに向いており、Engineering / Publishing の両モード、進捗表示、ログ、結果表示を備えています。自動化、スクリプト実行、バッチ処理には CLI を使ってください。
